...
uniform sampler2DRect tex0; // position
uniform sampler2DRect tex1; // position
uniform sampler2DRect tex2; // normals
uniform sampler2DRect tex3; // normals
...
void main() {
    ...
    vec4 c0 = vec4(texture2DRect(tex0, gl_MultiTexCoord0.st).xyz * scale, 1.0) + pos0;
    vec4 c1 = vec4(texture2DRect(tex1, texCoordVarying.st).xyz * scale, 1.0) + pos1;
    
    vec4 n0 = normalize(texture2DRect(tex2, texCoordVarying.xy)) * scale;
    vec4 n1 = normalize(texture2DRect(tex3, texCoordVarying.xy)) * scale;
